...
class image_converter:
    ...
    def rayToWorld(self, ray, ar_marker):
    ...
    def ic_setup(self):
        ...
        while run_while:
            try:
                ...
                self.trans5 = self.tfBuffer.lookup_transform("reference/base", "reference/left_hand_camera", rospy.Time())
                ...
                pose_base = tf2_geometry_msgs.do_transform_pose(box, self.trans4)
    ...
    def camToWorld(self, pixel):
        ...
        while not rospy.is_shutdown() and run_while:
            try:
                ...
                pose_pix = self.rayToWorld(ray, self.ar_marker)
                pose_lmao = tf2_geometry_msgs.do_transform_pose(pose_pix, self.trans5)
    ...
    def getStopPosition(self):
        self.desiredPixel = (None, 0)
        pixel1, time1 = self.getDesiredPixel()
        pixel2, time2 = self.getDesiredPixel()
        while time2-time1 < self.time_delta:
            pixel2, time2 = self.getDesiredPixel()

        pixel1_world = self.camToWorld(pixel1)
        ...
        time3 = time2 - time1
        velocity = (pixel3_world[0]/time3, pixel3_world[1]/time3)

        x_displacement = velocity[0] * self.time_to_stop
        y_displacement = velocity[1] * self.time_to_stop
        final_world = [pixel1_world[0] + x_displacement, pixel1_world[1] + y_displacement, pixel1_world[2]]
        return final_world, time1
